...
**Latest commit:** see the tip of this branch — the §16.4 A/B round (`95dfff8`) plus the §16.3a bullet-2 shell/tooling tasks on top of it
**PR:** [#18](https://github.com/Derric01/ClutchCode/pull/18) — **open**, carrying both.
...
**Test suite (locally):** 823/823 passing, 85 test files, clean `tsc -b`, clean
...
823 across 85 files now; the 758/16/774 split is run #12's, not today's.) The 16
...
| Hermes tool-call format for open-weight models | §4.7/§4.9 | small–medium, **DO FIRST** (the top genuinely-ungated row: 87 needs an infra/security-posture call on a privileged runner, 88 needs a macOS/Windows host, 89 needs a §20 boundary decision, 90 needs npm scope ownership) | **Confirmed gap, studied at the source.** `ollama.ts` sets `supportsNativeTools: false` and routes everything through `OpenAICompatibleProvider`, which parses only OpenAI-shaped `delta.tool_calls`. A local model emitting the Hermes shape — `<tool_call>{"name":…,"arguments":{…}}</tool_call>`, tools declared in the system prompt inside `<tools></tools>`, results returned in `<tool_response>` — is read as **plain prose** (verified by grep: no `<tool_call>` handling exists anywhere in `packages/providers`). The §4.9 probe then scores it `toolTransport: "none"`/`"json-mode-only"`, which cascades into a weaker edit format and a worse context budget, so a genuinely capable local model is systematically under-rated. This is the de-facto open-weight standard (vLLM ships `--tool-call-parser hermes`). **Fits ADR-015, does not fight it:** transport is *probed*, not a static per-model table. **Do NOT port their extractor** — `utils.py` wraps the whole assistant message in a synthetic `<root>` and strict-XML-parses it, which for a *coding* agent breaks on any message containing bare `<`, `&`, generics or JSX, discarding valid calls. Scan for `<tool_call>` regions lexically, JSON-parse only the region contents, never XML-parse surrounding prose, treat a bad region as one failed call not a failed message. Also handle Hermes-3's optional `<scratch_pad>` reasoning block (strip, don't leak) and multiple calls per turn. Streaming has **no reference upstream** (their parser is whole-message only) — incremental extraction across `sse.ts` chunk boundaries is ours to design. Their `chat_templates/*.j2` are usable as **test fixtures**. License: MIT, format is REUSE-eligible like MCP/ACP; prompt text is not (ADR-016). Study note: `research/repos/hermes-function-calling.md`. **Cannot be verified end-to-end here** (no Ollama, no GPU, no weights): unit-test the parser against captured/templated payloads, and flag the live-model claim as unverified in a header comment and README rather than asserting it. |
| Coordinated npm release (`npx clutchcode`) | §18/§21 | medium, **gated on you** (the top row nothing external blocks — the three rows above need, in order: an infra/security-posture call on a privileged runner, a macOS/Windows host, and a §20 boundary decision) | **The last adoption blocker.** Publish metadata now exists on all 11 manifests (`files`, `publishConfig.access: public`, `engines`, `repository.directory`, `bugs`, `homepage`) — what's missing is the release itself. `@clutchcode/cli` depends on `@clutchcode/agent-api` and `@clutchcode/agent-rpc` via `workspace:*`, which transitively requires publishing **all ten** workspace packages as one coordinated release, so this is a release *process*, not a publish command: decide the npm org/scope ownership, pick a versioning strategy (lockstep vs. independent — lockstep is simpler for a pre-1.0 monorepo where the packages only ever ship together), verify `pnpm publish -r` rewrites `workspace:*` to real ranges, add a release workflow gated on the CI gate, and confirm `npx clutchcode --help` works from a clean machine. Deliberately not half-started: partial publishes of an interdependent scope are worse than none. Until this lands the README quickstart correctly says "build from source". | **Where to stop (new):** exactly one step inside this row needs a human — **who owns the `@clutchcode` npm scope, and the token**. Everything before it is mechanical and worth doing in one unit: pick and record the versioning strategy (lockstep is right for a pre-1.0 monorepo whose packages only ever ship together), verify against a real registry-less dry run that `pnpm publish -r` rewrites `workspace:*` to concrete ranges, add a release workflow gated on the existing CI gate, and prove `npx clutchcode --help` works from a clean checkout+pack. Then **stop and ask** rather than publishing — and do not publish a partial scope, which is worse than not publishing.
