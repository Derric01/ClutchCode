...
**Test suite (locally):** 860/860 passing, 87 test files, clean `tsc -b`, clean
...
| Coordinated npm release (`npx clutchcode`) | §18/§21 | small now, **BLOCKED on two answers from the user** | **The mechanical half is done and verified** (see the log entry): lockstep confirmed (all 11 publishable manifests at `0.1.0`, vscode correctly `private`); `pnpm pack` **proven** to rewrite `workspace:*` to a pinned `0.1.0` by reading the packed manifest back out of the tarball; all 11 tarballs installed into a clean directory as `file:` deps with no repo on the path, and the npm-generated `.bin/clutchcode` shim runs — `--help` renders, and `doctor` returns exit 0 while exercising sandbox/keychain/toolchain across ten packages, which is the real proof `npx clutchcode` would work. External deps confirmed as exactly `ajv`, `commander`, `smol-toml`; 0 vulnerabilities. **Two answers are needed before anything else happens:** (1) **who owns the `@clutchcode` npm scope** and which account publishes; (2) **how the release workflow authenticates** — a long-lived `NPM_TOKEN` secret vs npm OIDC trusted publishing — plus the trigger policy (tag push vs manual dispatch). The workflow was deliberately **not written**: it is a security-posture change that cannot be verified here and would first truly run on a real publish, so leaving an unchosen publishing path in the repo is worse than leaving the row open. After those answers this is one workflow file plus `pnpm publish -r`. |
