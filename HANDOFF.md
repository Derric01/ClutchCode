...
**Latest commit:** see the tip of this branch — `b85ac37` — the §16.4 A/B round (`95dfff8`), the §16.3a bullet-2 shell/tooling tasks, the §4.7/§4.9 Hermes tool-call format (`c6ee7f1`), and a raw-NUL source-encoding fix with a hygiene guard
...
**Test suite (locally):** 856/856 passing, 87 test files, clean `tsc -b`, clean
...
854 across 86 files now; the 758/16/774 split is run #12's, not today's.) The 16
...
|---|---|---|---|
| Finish the `runAbComparison` logic review | §16.4 | small, **DO FIRST** (a deferred item, which `CLAUDE.md`'s loop takes ahead of the table proper) | **Interrupted mid-investigation, not concluded.** A run was scoping whether the A/B pairing in `evals/src/ab.ts` had a *logic* problem when it hit a rate limit; only the **encoding** issue it stumbled on (a literal NUL byte, fixed in `b85ac37`, class now guarded by `tests/source-hygiene.test.ts`) got resolved. The open question is the one it never answered: `runEvalTask` **refuses** a task whose declared `requires` are unusable on this host, and `computeAbReport` validates pairing — so confirm what happens when a task is refused in one arm but not the other, and whether an unpaired or dropped task can silently skew the published delta instead of failing loudly. That matters because §16.4's delta is the North Star claim. Read the pairing/balance validators first — they may already cover it, in which case close this row and say so rather than inventing work. |
